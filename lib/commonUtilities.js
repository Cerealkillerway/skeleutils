...
SkeleUtils.GlobalUtilities.skeleSetPanelBackground = function() {
    $('body').alterClass('siteBody', 'skelePanelBody');
};
...
SkeleUtils.GlobalUtilities.getFieldInstance = function(formInstance, name) {
    for (field of formInstance.Fields) {
        if (field.data.schema.name === name) {
            return field;
        }
    }
...
SkeleUtils.GlobalUtilities.refreshField = function(fieldInstance) {
    fieldInstance.forcedReloads.set(fieldInstance.forcedReloads.get() + 1);
}
