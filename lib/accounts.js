...
SkeleUtils.Accounts.loginWithPassword = function(email, password, redirectPath) {
    ...
    Meteor.loginWithPassword(email, password, function(error) {
        if (error) {
            switch (error.reason) {
                case 'User not found':
                message = Skeletor.Skelelang.i18n.get('userNotFound_error');
                break;

                case 'Incorrect password':
                message = Skeletor.Skelelang.i18n.get('incorrectPassword_error');
                break;
            }

            Materialize.toast(Skeletor.Skelelang.i18n.get('accessDenied_error') + ' - ' + message, 4000);
        }
        else {
